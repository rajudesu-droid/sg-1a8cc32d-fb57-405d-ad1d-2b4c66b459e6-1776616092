...
export class CurveAdapter extends BaseProtocolAdapter {
  public protocolName = "Curve";
  public protocolType = "standard_amm_dex" as ProtocolType;
  public supportedChains = ["ethereum", "arbitrum", "polygon", "optimism", "avalanche"];
  ...
  async openPosition(chain: string, poolAddress: string, params: PositionParams): Promise<string> {
    throw new Error(`[Curve] Adapter not approved for execution in live mode`);
  }
  ...
  async normalizeOpportunity(chain: string, poolAddress: string): Promise<NormalizedOpportunity> {
    ...
    return {
      ...
      chain, poolAddress, poolType: "stable_amm",
