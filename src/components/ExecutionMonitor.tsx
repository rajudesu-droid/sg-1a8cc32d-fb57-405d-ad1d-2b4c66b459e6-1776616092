...
export function ExecutionMonitor() {
  ...
  const getReconciliation = (job: any) => {
    return job.executionResult?.reconciliation;
  };

  const hasDriftWarning = (job: any) => {
    const recon = getReconciliation(job);
    return recon && recon.criticalDiscrepancies > 0;
  };
            ...
            {completedJobs.length > 0 && (
              <div className="space-y-3">
                ...
                {completedJobs.map((job) => {
                  const recon = getReconciliation(job);
                  const hasDrift = hasDriftWarning(job);
                  
                  return (
                    <div 
                      key={job.id}
                      className="border border-border rounded-lg p-3 opacity-75 hover:opacity-100 transition-opacity space-y-2"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {getStatusIcon(job.status)}
                        ...
                        <Alert className="border-amber-500/50 bg-amber-500/10">
                          ...
                          <AlertDescription className="text-xs">
                            ...
                            <div className="mt-1 space-y-1">
                              <div>Total discrepancies: {recon.totalDiscrepancies}</div>
                              <div>Critical: {recon.criticalDiscrepancies}</div>
                              {recon.requiresUserAttention && (
                      ...
                      {recon && !hasDrift && job.mode === "live" && (
                        <div className="text-xs text-green-400 flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" />
                          <span>State verified onchain</span>
                        </div>
                      )}
                    </div>
                  );
